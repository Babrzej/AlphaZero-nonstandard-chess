...
def board_game_train_Q(game_object, players_to_train, strategy_w=None, strategy_b=None, number_of_games = 1000, choose_random = []):
    ...
    for game_nr in range(number_of_games):                # episodes loop
        ...
        player = 1                                        # first movement by player 1 (white)
...
def experiment_par_train():
    ...
    game = bfun.Chess("szachy_plansza_5x5")
    ...
    if if_Q_from_file:
        try:            
            strategy_w.from_file('Q_w_'+game.initial_board_name+'.pkl')
        except:
            pass
        try:
            strategy_b.from_file('Q_b_'+game.initial_board_name+'.pkl')
        except:
            pass
    ...
    if if_Q_save_to_file:
        strategy_w.to_file('Q_w_'+game.initial_board_name+'.pkl')
        strategy_b.to_file('Q_b_'+game.initial_board_name+'.pkl')
