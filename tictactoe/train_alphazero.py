...
def main():
    ...
    print("="*50)
    print("ROZPOCZĘCIE TRENINGU ALPHAZERO")
    print("="*50)

    for episode in range(episodes):
        print(f"\n--- Generacja {episode+1}/{episodes} ---")
        
        net.eval()
        all_training_data = []

        print("Rozgrywanie partii (Self-Play)...", end="", flush=True)
        for g in range(games_per_ep):
            game_data = play_game(game, net, mcts_simulations=mcts_sims, temp_moves=temp_moves)
            all_training_data.extend(game_data)
            print(".", end="", flush=True)
        print(f" Zebrano {len(all_training_data)} pozycji.")
        ...
        print(f"Bufor replay: {len(replay_buffer)} pozycji | Trening na: {len(sampled_data)} pozycji")
        ...
        if (episode + 1) % 10 == 0:
            ...
            print(f"✓ Zapisano checkpoint: alphazero_tic_tac_toe_gen{episode+1}.pth")
